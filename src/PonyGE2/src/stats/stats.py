...
def update_stats(individuals, end):
    ...
    if end:
        ...
        for ind_dic in ind_sample:
            ...
            ind.novelty = np.NaN
        with open("derivation_novelties.txt", "w+") as dfile:
            for dval in derivation_novelties:
                dfile.write(str(dval) + '\n')

        with open("output_novelties.txt", "w+") as ofile:
            for oval in output_novelties:
                ofile.write(str(oval) + "\n")
